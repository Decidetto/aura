...
## ЭТАП 6 — Закрытие findings повторного аудита (выполнен)

Отдельный проход (см. `audit_report.md`) с выделением последствий: 14 backend (C1–C14)
и 6 frontend (F1–F6) findings.

### C‑находки (4 коммита)
- `fc61a78` — C1, C2: повторный `start` больше не инвалидирует живую сессию; модификатор
  не срабатывал из‑за order-бага → мод-статика 48 Ghost- горячие клавиши.
- `8f54b1a` — C3–C5: сброс залипших модификаторов; облачные таймауты 900 с;
  потолок длительности сессии Parakeet 1200 с.
- `f190b99` — C6–C14: зеркало после отравленных мьютексов; clipboard‑mutex на копию
  контекста; SHA‑256 контроль моделей; retry‑warm‑up; `merge_transcripts` дедупликация;
  watchdog без паники; дедлайн загрузок; tail‑drain 2 с; хуки с заменяемыми коллбеками.
- `3cb7975` — F1–F6 (см. ниже).

### F1–F6 (frontend)
- F1: `refreshDownloadedModels` не срывает UI активной загрузки (`inFlightModelDownloads`).
- F2: оверлей-hidden failsafe‑таймер + `.catch`.
- F3: guard по `stateCycle` — поздний hide старой сессии не маскирует новую.
- F4: `.catch` на minimize/close/drag IPC.
- F5: ключи провайдеров сохраняются по одному — сбой одного не прерывает остальных.
- F6: hotkey-prompt, placeholder'ы API-ключа и «не сохранено» в i18n (9 локалей).

**Верификация этапа 6:** `cargo test` 105/105, clippy 0 новых warning, `node --check`
и typecheck чисты, frontend-статические 10/10, `cargo build --release` ✅.
...
6. **Этап 5** (проверка) →
7. **Этап 6** (выполнен: C1–C14 + F1–F6, закрытие повторного аудита).
