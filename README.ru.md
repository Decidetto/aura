...
- **Два режима распознавания**:
  - **Локальный (100% офлайн и приватно)** — whisper.cpp или NVIDIA Parakeet TDT v3 (sherpa-onnx) на CPU или GPU NVIDIA CUDA. Звук не покидает ваш компьютер; модели скачиваются в один клик прямо из настроек.
...
- **Выбор аудиоустройства и защита от обрыва фраз** — выбор физического микрофона, 160-мс пост-релизный буфер удержания звука и Silero VAD для предотвращения обрезания окончаний слов.
...
> Честные компромиссы: официальные сборки Aura пока доступны только для Windows 10 и 11 (порт для macOS уже в кодовой базе). Handy — зрелое кроссплатформенное решение. Wispr Flow предлагает вылизанный интерфейс, но работает исключительно через платную подписку и закрытое облако.
...
- **Выбор аудиоустройства и VU-метр** — выбор физического микрофона прямо в настройках с индикатором уровня сигнала и Silero VAD детекцией в реальном времени.
- **Защита от обрыва окончаний (Tail Hold)** — 160-мс аудио-буфер удержания звука после отпускания клавиши и расширенное окно Silero VAD, полностью устраняющие проглатывание последних слогов.
...
- **Мгновенное скрытие оверлея** — исключена задержка скрытия: звуковой сигнал и анимация закрытия срабатывают сразу при вставке текста.
